...
# entries are the O45-v2 5.4.300->5.4.301 conflict resolutions frozen by O46-v3.
HOST_FREEZE = (
    "fs/open.c",
    "fs/stat.c",
    "fs/exec.c",
...
O45_301_FROZEN_SHA256 = {
    "drivers/base/regmap/regmap.c": "892df435693b2c0b386107b4862d93cfa4e1861de4d62b41af0dec3afa61cd27",
    "drivers/media/pci/ivtv/ivtv-streams.c": "dcd2ea466fe31d79fb227da06463d4c871ed99bddd884a3bc911eff5ff50bab4",
    "drivers/soc/qcom/rpmh-rsc.c": "845007d3c7a7cc3975f8750dfc34a88eb6db318a10f6285b1948fabca9086acb",
}
...
def die(msg: str) -> None:
    print(f"LOCAL_301_STAGE_ERROR: {msg}", file=sys.stderr)
    raise SystemExit(1)
...
def patch_pinned_kbuild(path: Path) -> None:
    ...
    replacement = (
        "# VEUX 5.4.301: authenticated local ReSukiSU 35170 snapshot.\n"
...
def verify_301_host(src: Path) -> None:
    for rel, expected in O45_301_FROZEN_SHA256.items():
        p = src / rel
        if not p.is_file():
            die(f"O45-v2 frozen 5.4.301 host file missing: {rel}")
        got = sha256(p)
        if got != expected:
            die(f"O45-v2 frozen 5.4.301 host drift: {rel}: {got} != {expected}")
    ivtv = (src / "drivers/media/pci/ivtv/ivtv-streams.c").read_text(encoding="utf-8")
    if any(x in ivtv for x in ("PCI_DMA_FROMDEVICE", "PCI_DMA_TODEVICE", "PCI_DMA_NONE")):
        die("O45-v2 IVTV DMA union regression")
    rpmh = (src / "drivers/soc/qcom/rpmh-rsc.c").read_text(encoding="utf-8")
    if "__tcs_trigger(drv, i, false);" not in rpmh:
        die("O45-v2 RPMH trigger resolution missing")
    if "ksu_handle_post_execve" not in (src / "fs/exec.c").read_text(encoding="utf-8"):
        die("P13 post-exec hook missing from 5.4.301 host")
    if "needs_force_resume:1;" not in (src / "include/linux/pm.h").read_text(encoding="utf-8"):
        die("5.4.301 PM dependency closure missing")
    smci = (src / "drivers/soc/qcom/smcinvoke.c").read_text(encoding="utf-8")
    if "smci_size_add" not in smci:
        die("5.4.301 timer/sizeadd dependency closure missing")
    print("O45_301_FROZEN_HOST=PASS")
...
def verify_baseline(src: Path) -> None:
    ...
    if not vendor.is_dir() or not (vendor / ".git").exists():
        die("O45/O51 5.4.301 KernelSU baseline tree missing")
    ...
    print("NATIVE_301_BASELINE_LAYOUT=PASS")
...
def integrate_nomount(src: Path, snapshot: Path) -> None:
    ...
    if before != after:
        ...
        die(f"NoMount changed frozen 5.4.301 host surface: {changed}")
    print("NOMOUNT_2_0_0_NATIVE_301_STAGE=PASS")
...
def stage(src: Path, resukisu: Path, nomount: Path) -> None:
    ...
    if before != after:
        changed = sorted(k for k in before if before[k] != after[k])
        die(f"ReSukiSU changed frozen 5.4.301 host surface: {changed}")
    ...
    print("LOCAL_301_TARGET_STAGE=PASS")
...
def verify_target(src: Path) -> None:
    ...
    print("NATIVE_301_TARGET_VERIFY=PASS")
...
def verify_topology(src: Path, report: Path) -> None:
    ...
    with report.open("w", encoding="utf-8") as f:
        ...
        f.write("PORT301_KSU_DUAL_PATH_BINDING=PASS\n")
    print("PORT301_KSU_DUAL_PATH_BINDING=PASS")


def selftest() -> None:
    with tempfile.TemporaryDirectory(prefix="veux-301-native-") as td:
        root = Path(td)
        kbuild = root / "Kbuild"
        kbuild.write_text(
    ...
    print("LOCAL_301_STAGE_SELFTEST=PASS")
