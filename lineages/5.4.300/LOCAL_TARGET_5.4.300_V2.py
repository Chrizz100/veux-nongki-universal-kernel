...
def baseline_35158_compat_gate(vendor: Path) -> None:
    # EXACTLY the compatibility surface already proven by the O43/O51 35158
    # baseline and re-observed by R6 before it reached the old target-only check.
    checks = {
...
def target_35170_compat_gate(vendor: Path) -> None:
    # Target-only checks belong here, AFTER staging the authenticated 35170
    # snapshot. Never use these to validate the older 35158 baseline.
    checks = {
        "kernel/compat/kernel_compat.c": (
            "struct key *init_session_keyring = NULL;",
            "install_session_keyring_to_cred(ksu_cred, init_session_keyring);",
...
def selftest() -> None:
    with tempfile.TemporaryDirectory(prefix="veux-300-native-") as td:
        ...
        # Classification regression test: a valid 35158 baseline intentionally
        # lacks the two 35170-only files/markers and MUST still pass baseline.
        src = root / "baseline-src"
        v = src / "KernelSU"
        (v / "kernel/compat").mkdir(parents=True)
        (v / "kernel/hook").mkdir(parents=True)
        (v / "kernel/core").mkdir(parents=True)
        (v / "uapi").mkdir(parents=True)
        (v / ".git").write_text("gitdir: synthetic\n", encoding="utf-8")
        (v / "kernel/Kbuild").write_text("baseline\n", encoding="utf-8")
        (v / "uapi/supercall.h").write_text(
            "static const __u32 KERNEL_SU_UAPI_VERSION = 4;\n", encoding="utf-8"
        )
        (v / "kernel/compat/kernel_compat.c").write_text(
            "struct key *init_session_keyring = NULL;\n"
            "install_session_keyring_to_cred(ksu_cred, init_session_keyring);\n",
            encoding="utf-8",
        )
        (v / "kernel/hook/lsm_hooks.c").write_text(
            "LSM_HOOK_INIT(key_permission, ksu_handle_key_permission)\n",
            encoding="utf-8",
        )
        (v / "kernel/core/init.c").write_text(
            "init_session_keyring = ksu_get_session_keyring(current_cred());\n",
            encoding="utf-8",
        )
        verify_baseline(src)
        if (v / "kernel/compat/kernel_compat.h").exists() or (v / "kernel/manager/manager.c").exists():
            die("selftest accidentally supplied target-only baseline files")

        # Target classification test: add ONLY the 35170-only evidence and the
        # target identity/NoMount postconditions, then verify the target gate.
        (v / "kernel/manager").mkdir(parents=True)
        (v / "kernel/compat/kernel_compat.h").write_text(
            "ksu_filp_open_nonotify\n", encoding="utf-8"
        )
        (v / "kernel/manager/manager.c").write_text(
            "void ksu_unregister_all_manager(void)\n", encoding="utf-8"
        )
        (v / "kernel/Kbuild").write_text(
            "KSU_LOCAL_VERSION := 4470\n"
            "KSU_VERSION := 35170\n"
            "KSU_COMMIT_SHA := 9be0f347\n"
            "KSU_BRANCH_NAME := main\n",
        ...
        (src / "fs/nomount").mkdir(parents=True)
        (src / "fs/nomount/nomount.c").write_text("/* synthetic */\n", encoding="utf-8")
        (src / "fs/Kconfig").write_text('source "fs/nomount/Kconfig"\n', encoding="utf-8")
        (src / "fs/Makefile").write_text("obj-$(CONFIG_NOMOUNT) += nomount/\n", encoding="utf-8")
        verify_target(src)

    print("BASELINE_TARGET_CLASSIFICATION_SELFTEST=PASS")
