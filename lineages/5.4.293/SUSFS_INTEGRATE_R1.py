...
# Exact blob preimages for the two VEUX-specific files that failed the
# generic donor patch in GitHub Actions run 35344193301.  V2 refuses to
# port these files unless the source bytes are exactly the audited ones.
VEUX_TASK_MMU_BLOB = "5b83bcd5e7e5e92394b3391cfcc73cf1fca83230"
VEUX_KERNEL_SYS_BLOB = "eb21538edb3eba7879d0fbfa79aef9024336e873"

# The 5.4.293 stable delta does not touch these audited hook surfaces.
HOOK_SURFACE_BLOBS = {
    "fs/exec.c": "dff202eabce643714873ca7ff5f7ec867f000076",
    "fs/open.c": "5c0043d4b8072e9e86a94281788ea6f50b507721",
    "fs/read_write.c": "93d06b4c5f78c7430e5c3314072bc2a6817d8f96",
    "fs/stat.c": "298eb77668a766cf9feafa586bba613a6fe19eeb",
    "drivers/input/input.c": "45fdb9bdf08d42c9a6efda70e9422a16a2d1c1a3",
    "kernel/reboot.c": "8f08af3a7d04fe4dc446af0bf14d976aec02a272",
    "kernel/sys.c": VEUX_KERNEL_SYS_BLOB,
}
...
def replace_once_in_braced_block(path: Path, start_marker: str, old: str, new: str, label: str) -> None:
    """Replace exactly once inside one C braced block selected by a unique marker."""
    data = read(path)
    marker_count = data.count(start_marker)
    if marker_count != 1:
        die(f"{label}: expected exactly one block marker in {path}, found {marker_count}")
    start = data.index(start_marker)
    brace = data.find("{", start + len(start_marker))
    if brace < 0:
        die(f"{label}: opening brace not found in {path}")
    depth = 0
    end = -1
    for i in range(brace, len(data)):
        ...
        if ch == "{":
            ...
            if depth == 0:
                ...
                break
    if end < 0:
        die(f"{label}: closing brace not found in {path}")
    block = data[start:end]
    count = block.count(old)
    if count != 1:
        die(f"{label}: expected exactly one anchor inside selected block in {path}, found {count}")
    block = block.replace(old, new, 1)
    write(path, data[:start] + block + data[end:])
...
def git_blob(root: Path, rel: str) -> str:
    return run(["git", "-C", str(root), "rev-parse", f"HEAD:{rel}"], capture=True)


def verify_divergent_preimages(kernel: Path) -> None:
    expected = {
        "fs/proc/task_mmu.c": VEUX_TASK_MMU_BLOB,
        "kernel/sys.c": VEUX_KERNEL_SYS_BLOB,
    }
    for rel, want in expected.items():
        got = git_blob(kernel, rel)
        if got != want:
            die(f"VEUX divergent preimage mismatch for {rel}: expected {want}, got {got}")


def verify_hook_surface_preimages(kernel: Path) -> None:
    for rel, want in HOOK_SURFACE_BLOBS.items():
        got = git_blob(kernel, rel)
        if got != want:
            die(f"SUSFS hook-surface preimage mismatch for {rel}: expected {want}, got {got}")
...
def backport_clone_mnt_race_fix(kernel: Path) -> None:
    """Backport the proven SUSFS clone_mnt race fix from the 5.4.292 V5 line."""
    path = kernel / "fs/namespace.c"
    data = read(path)

    fn_start = data.find("static struct mount *clone_mnt(")
    if fn_start < 0:
        die("clone_mnt race-fix: clone_mnt function not found")
    fn_end = data.find("\nstatic ", fn_start + 1)
    if fn_end < 0:
        fn_end = len(data)
    region = data[fn_start:fn_end]

    for item in (
        "bool is_mnt_ksu_unshared = false;",
        "is_mnt_ksu_unshared = true;",
    ):
        if region.count(item) != 1:
            die(f"clone_mnt race-fix precondition mismatch for {item!r}: {region.count(item)}")

    flags_anchor = (
        "\tmnt->mnt.mnt_flags = old->mnt.mnt_flags;\n"
        "\tmnt->mnt.mnt_flags &= ~(MNT_WRITE_HOLD|MNT_MARKED|MNT_INTERNAL);\n\n"
    )
    tail_anchor = "\tatomic_inc(&sb->s_active);"
    if region.count(flags_anchor) != 1:
        die(f"clone_mnt race-fix flags anchor mismatch: {region.count(flags_anchor)}")
    if region.count(tail_anchor) != 1:
        die(f"clone_mnt race-fix tail anchor mismatch: {region.count(tail_anchor)}")

    a = region.index(flags_anchor) + len(flags_anchor)
    b = region.index(tail_anchor, a)
    middle = region[a:b]
    ...
    if "if (unlikely(is_mnt_ksu_unshared))" in middle:
        if middle != new_block:
            die("clone_mnt race-fix: new semantic block present but surrounding block is unexpected")
    else:
        old_markers = (
        ...
        missing = [x for x in old_markers if x not in middle]
        if missing:
            die("clone_mnt race-fix old semantic block mismatch: " + ", ".join(missing))
        if middle.count("mnt->mnt.mnt_flags |= VFSMOUNT_MNT_FLAGS_KSU_UNSHARED_MNT;") != 1:
            die("clone_mnt race-fix old marker count mismatch")
        region = region[:a] + new_block + region[b:]
        data = data[:fn_start] + region + data[fn_end:]
        write(path, data)

    final = read(path)
    fn_start = final.find("static struct mount *clone_mnt(")
    fn_end = final.find("\nstatic ", fn_start + 1)
    if fn_end < 0:
        fn_end = len(final)
    final_region = final[fn_start:fn_end]
    if final_region.count("if (unlikely(is_mnt_ksu_unshared))") != 1:
        die("clone_mnt race-fix postcondition missing captured boolean check")
    if final_region.count("static_branch_unlikely(&susfs_is_sdcard_android_data_not_decrypted)") != 1:
        die("clone_mnt race-fix unexpectedly changed allocation-path static-key check")
    if final_region.count("mnt->mnt.mnt_flags |= VFSMOUNT_MNT_FLAGS_KSU_UNSHARED_MNT;") != 1:
        die("clone_mnt race-fix postcondition marker count mismatch")
...
DIVERGENT_DONOR_FILES = (
    "fs/proc/task_mmu.c",
    "kernel/sys.c",
)


def strip_patch_file_sections(text: str, excluded: tuple[str, ...]) -> str:
    """Remove complete git-diff sections for VEUX-divergent files.

    The V1 failure (GitHub run 35344193301) proved that these files cannot be
    passed through the donor's generic 5.4 patch.  They are ported below with
    exact VEUX anchors instead.
    """
    prefix = "diff --git "
    if not text.startswith(prefix):
        raise ValueError("donor patch does not start with a git diff section")
    ...
    for raw in parts[1:]:
        ...
        if not sep:
            raise ValueError("malformed donor patch section")
        fields = header.split()
        if len(fields) < 2 or not fields[0].startswith("a/") or not fields[1].startswith("b/"):
            raise ValueError(f"malformed donor patch header: {header}")
        path_a = fields[0][2:]
        path_b = fields[1][2:]
        if path_a != path_b:
            raise ValueError(f"unexpected rename in donor patch: {header}")
        if path_a in excluded:
            removed.append(path_a)
            continue
        out.append(prefix + raw)
    if tuple(removed) != excluded:
        raise ValueError(f"divergent donor section removal mismatch: expected {excluded}, got {tuple(removed)}")
    filtered = "".join(out)
    for rel in excluded:
        if f"diff --git a/{rel} b/{rel}" in filtered:
            raise ValueError(f"excluded donor section leaked into filtered patch: {rel}")
    return filtered


def port_task_mmu_veux(kernel: Path) -> None:
    """Port the donor task_mmu SUSFS hunks onto the exact VEUX 5.4.292 tree.

    VEUX carries a device-specific maps rewrite for mappings whose dentry name
    contains 'lineage'.  The generic donor hunk cannot apply there, and that
    custom behavior must be preserved rather than overwritten.
    """
    path = kernel / "fs/proc/task_mmu.c"
    ...
    replace_once(
        ...
        "\t\t\tint susfs_ret = susfs_open_redirect_spoof_show_map_vma_srcu(inode, &ino, &dev, &spoofed_redirected_name);\n"
        "\t\t\tif (!susfs_ret) {\n"
        ...
        "#endif\n"
        "\t\tdev = inode->i_sb->s_dev;\n",
        "VEUX task_mmu map/open_redirect port",
    )
    ...
    replace_once(
        ...
        "#endif\n"
        "        struct dentry *dentry = file->f_path.dentry;\n",
        "VEUX task_mmu kstat port while preserving lineage mapping rule",
    )
    ...
    replace_once(
        ...
        "#endif\n"
        "\t\tlast_vma_end = vma->vm_end;\n\t}\n",
        "VEUX task_mmu smaps_rollup port",
    )
    ...
    replace_once(
        ...
        "\tunsigned long end_vaddr;\n\tint ret = 0, copied = 0;\n"
        "#ifdef CONFIG_KSU_SUSFS_SUS_MAP\n"
        "\tstruct vm_area_struct *susfs_vma;\n"
        "#endif\n\n"
        "\tif (!mm || !mmget_not_zero(mm))\n",
        "VEUX task_mmu pagemap state",
    )
    ...
    replace_once(
        ...
        "\t\tret = down_read_killable(&mm->mmap_sem);\n\t\tif (ret)\n\t\t\tgoto out_free;\n"
        "#ifdef CONFIG_KSU_SUSFS_SUS_MAP\n"
        "\t\tsusfs_vma = find_vma(mm, start_vaddr);\n"
        "\t\tif (susfs_vma && susfs_vma->vm_file && SUSFS_IS_INODE_SUS_MAP(file_inode(susfs_vma->vm_file)))\n"
        "\t\t\tgoto susfs_pagemap_unlock;\n"
        "#endif\n"
        "\t\tret = walk_page_range(mm, start_vaddr, end, &pagemap_ops, &pm);\n"
        "#ifdef CONFIG_KSU_SUSFS_SUS_MAP\n"
        "susfs_pagemap_unlock:\n"
        "#endif\n"
        "\t\tup_read(&mm->mmap_sem);\n",
        "VEUX task_mmu pagemap port",
    )
    ...
    for marker in (
        ...
        if marker not in data:
            die(f"VEUX task_mmu postcondition missing: {marker}")


def port_kernel_sys_veux(kernel: Path) -> None:
    """Port SUSFS uname spoofing without deleting VEUX's netmgrd qgki rule."""
    ...
    replace_once_in_braced_block(
        ...
        "#endif\n"
        "\tup_read(&uts_sem);\n",
        "VEUX newuname SUSFS call",
    )
    data = read(path)
    if 'is_netmgrd = !strcmp(t->comm, "netmgrd");' not in data:
        die("VEUX uname port lost netmgrd qgki compatibility rule")
    if data.count("susfs_spoof_uname(&tmp);") != 1:
        die("VEUX uname SUSFS call count mismatch")
...
def apply_outer_patch(kernel: Path, donor_root: Path, susfs_root: Path) -> Path:
    ...
    filtered = strip_patch_file_sections(fixed, DIVERGENT_DONOR_FILES)

    tmpdir = Path(tempfile.mkdtemp(prefix="veux-susfs-patch-"))
    fixed_patch = tmpdir / "susfs_patch_to_5.4.corrected.filtered.patch"
    write(fixed_patch, filtered)

    # Run 35344193301 proved the generic donor hunks for task_mmu.c and
    # kernel/sys.c do not apply to the exact VEUX tree.  The filtered patch is
    # still checked fail-closed; the two excluded files are then ported with
    # exact, device-specific anchors below.
    run(["git", "-C", str(kernel), "apply", "--check", "--whitespace=fix", str(fixed_patch)])
    run(["git", "-C", str(kernel), "apply", "--whitespace=fix", str(fixed_patch)])
    port_task_mmu_veux(kernel)
    port_kernel_sys_veux(kernel)
    ...
    backport_clone_mnt_race_fix(kernel)
...
def verify_resukisu_manager_rcu_fix(kernel: Path) -> None:
    """Verify the 2026-09-19 ReSukiSU manager RCU/uaccess fix is present and ordered safely."""
    path = kernel / ".resukisu/kernel/manager/manager.c"
    data = read(path)
    start_marker = "int ksu_handle_get_managers_cmd(struct ksu_get_managers_cmd __user *arg, struct ksu_get_managers_cmd *cmd)\n"
    if data.count(start_marker) != 1:
        die(f"ReSukiSU manager RCU fix: function marker count mismatch: {data.count(start_marker)}")
    start = data.index(start_marker)
    brace = data.find("{", start + len(start_marker))
    if brace < 0:
        die("ReSukiSU manager RCU fix: opening brace not found")
    depth = 0
    end = -1
    for i in range(brace, len(data)):
        ch = data[i]
        if ch == "{":
            ...
            if depth == 0:
                ...
                break
    if end < 0:
        die("ReSukiSU manager RCU fix: closing brace not found")
    block = data[start:end]
    ...
    required = (
        ...
        "copy_to_user((char __user *)arg + sizeof(*cmd), entries, copy_count * sizeof(*entries))",
        "kfree(entries);",
    )
    for item in required:
        if item not in block:
            die(f"ReSukiSU manager RCU fix contract missing: {item}")

    lock = block.find("rcu_read_lock();")
    unlock = block.find("rcu_read_unlock();")
    copy = block.find("copy_to_user(")
    if lock < 0 or unlock < 0 or copy < 0:
        die("ReSukiSU manager RCU fix ordering markers missing")
    if not (lock < unlock < copy):
        die("ReSukiSU manager RCU fix ordering invalid: copy_to_user must be after rcu_read_unlock")
    if "copy_to_user(" in block[lock:unlock]:
        die("ReSukiSU manager RCU fix regression: copy_to_user remains inside RCU read-side critical section")
    if "struct ksu_manager_entry entry =" in block:
        die("ReSukiSU manager RCU fix regression: legacy per-entry user copy path remains")
...
def semantic_gates(kernel: Path) -> None:
    ...
    compat_h = read(kernel / ".resukisu/kernel/compat/kernel_compat.h")
    sucompat_c = read(kernel / ".resukisu/kernel/feature/sucompat.c")
    runtime_c = read(kernel / ".resukisu/kernel/runtime/ksud_integration.c")
    uapi_h = read(kernel / ".resukisu/uapi/supercall.h")
    if f"static const __u32 KERNEL_SU_UAPI_VERSION = {EXPECTED_KSU_UAPI};" not in uapi_h:
        die("ReSukiSU UAPI version contract missing")
    if (
        "#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 3, 0) || defined(KSU_HAS_MODERN_STATIC_KEY_INTERFACE)" not in compat_h
        or "#define KSU_COMPAT_USE_STATIC_KEY" not in compat_h
    ):
        die("ReSukiSU 5.4 static-key ABI contract missing")
    if "DEFINE_STATIC_KEY_TRUE(ksu_su_compat_enabled);" not in sucompat_c:
        die("ReSukiSU sucompat static-key definition missing")
    for item in (
        "DEFINE_STATIC_KEY_TRUE(ksu_is_init_rc_hook_enabled);",
        "DEFINE_STATIC_KEY_TRUE(ksu_is_input_hook_enabled);",
    ):
        if item not in runtime_c:
            die(f"ReSukiSU SUSFS static-key definition missing: {item}")

    verify_resukisu_manager_rcu_fix(kernel)

    arm64_kconfig = read(kernel / "arch/arm64/Kconfig")
    if "select THREAD_INFO_IN_TASK" not in arm64_kconfig:
        die("ARM64 THREAD_INFO_IN_TASK selection contract missing")

    selinuxfs_data = read(kernel / "security/selinux/selinuxfs.c")
    if "static ssize_t (*const write_op[])(struct file *, char *, size_t)" in selinuxfs_data:
        die("SELinux write_op remained static")
    if "ssize_t (*const write_op[])(struct file *, char *, size_t)" not in selinuxfs_data:
        die("SELinux write_op exported form missing")
    if "static const struct file_operations sel_handle_status_ops" in selinuxfs_data:
        die("SELinux sel_handle_status_ops remained static")
    if "const struct file_operations sel_handle_status_ops" not in selinuxfs_data:
        die("SELinux sel_handle_status_ops exported form missing")
...
def verify_real(project_root: Path, kernel: Path, resukisu: Path, donor: Path, susfs: Path, baseline_commit: str) -> Path:
    require_clean_git(kernel, baseline_commit, "kernel 5.4.293 validated baseline")
    ...
    parent = run(["git", "-C", str(kernel), "rev-parse", "HEAD^"], capture=True)
    if parent != SOURCE_COMMIT:
        die(f"kernel baseline parent mismatch: expected {SOURCE_COMMIT}, got {parent}")
    verify_divergent_preimages(kernel)
    verify_hook_surface_preimages(kernel)

    version = run(["make", "-s", "-C", str(kernel), "kernelversion"], capture=True)
    if version != KERNEL_VERSION:
        die(f"kernelversion mismatch: {version}")
    if not (kernel / "arch/arm64/configs" / DEFCONFIG).is_file():
        die(f"defconfig missing: {DEFCONFIG}")
    ...
    print("VEUX293_BASELINE_PIN=PASS")
    print(f"VEUX293_BASELINE_COMMIT={baseline_commit}")
    print(f"VEUX293_BASELINE_PARENT={parent}")
    return snapshot
...
def finalize_real(project_root: Path, kernel: Path, snapshot: Path) -> None:
    ...
    ksu_count = int(run(["git", "-C", str(vendor), "rev-list", "--count", "HEAD"], capture=True))
    ksu_version = 30000 + ksu_count + 700
    if ksu_version != EXPECTED_KSU_VERSION:
        die(f"ReSukiSU version-code mismatch: expected {EXPECTED_KSU_VERSION}, got {ksu_version}")
    ...
    print("UPSTREAM_CLONE_MNT_RACE_FIX_BACKPORT=PASS")
    print("SUSFS_5_4_CORE_COHERENCE=PASS")
    print("VEUX_DIVERGENT_PREIMAGE_GATE=PASS")
    print("HOOK_SURFACE_PREIMAGE_GATE=PASS")
    print("VEUX_DIVERGENT_OUTER_PORT=PASS")
    print("RESUKISU_STATIC_KEY_ABI=PASS")
    print("RESUKISU_RCU_COPY_TO_USER_FIX=PASS")
    print(f"RESUKISU_VERSION={ksu_version}")
    print(f"RESUKISU_UAPI={EXPECTED_KSU_UAPI}")
    print("ARM64_THREAD_INFO_CONTRACT=PASS")
    print("SELINUX_STATIC_EXPORT_CONTRACT=PASS")
    print("RUN35344193301_REGRESSION=PASS")
    print("RUN35346290137_REGRESSION=PASS")
    print("RUN35350262681_REGRESSION=PASS")
    print("RUN35352162500_REGRESSION=PASS")
    print("RUN35420076428_REGRESSION=PASS")
...
def selftest() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="susfs-v2-statfs-compile-selftest-") as td:
        root = Path(td)
    ...
    with tempfile.TemporaryDirectory(prefix="susfs-293-v3-clone-race-selftest-") as td:
        root = Path(td)
        (root / "fs").mkdir(parents=True)
        ...
        ns.write_text(
            ...
            "\treturn mnt;\n"
            "}\n\n"
            "static void after_clone(void) {}\n",
            encoding="utf-8",
        )
        backport_clone_mnt_race_fix(root)
        t = ns.read_text(encoding="utf-8")
        if "if (unlikely(is_mnt_ksu_unshared))" not in t:
            die("selftest clone_mnt race-fix backport missing")
        if t.count("mnt->mnt.mnt_flags |= VFSMOUNT_MNT_FLAGS_KSU_UNSHARED_MNT;") != 1:
            die("selftest clone_mnt race-fix marker count mismatch")

    with tempfile.TemporaryDirectory(prefix="susfs-v2-core-coherence-selftest-") as td:
        root = Path(td)
        k = root / "kernel"
        u = root / "upstream"
    ...
    # Regression for GitHub run 35344193301: the generic donor patch must
    # never again contain the two known VEUX-divergent file sections.
    patch_fixture = (
        "diff --git a/fs/Makefile b/fs/Makefile\n--- a/fs/Makefile\n+++ b/fs/Makefile\n@@ -1 +1 @@\n-a\n+b\n"
        "diff --git a/fs/proc/task_mmu.c b/fs/proc/task_mmu.c\n--- a/fs/proc/task_mmu.c\n+++ b/fs/proc/task_mmu.c\n@@ -1 +1 @@\n-a\n+b\n"
        "diff --git a/kernel/sys.c b/kernel/sys.c\n--- a/kernel/sys.c\n+++ b/kernel/sys.c\n@@ -1 +1 @@\n-a\n+b\n"
    )
    stripped = strip_patch_file_sections(patch_fixture, DIVERGENT_DONOR_FILES)
    if "fs/proc/task_mmu.c" in stripped or "kernel/sys.c" in stripped or "fs/Makefile" not in stripped:
        die("selftest divergent donor filtering failed")
    with tempfile.TemporaryDirectory(prefix="susfs-v2-filtered-git-apply-selftest-") as td:
        repo = Path(td)
        (repo / "fs/proc").mkdir(parents=True)
        (repo / "kernel").mkdir(parents=True)
        (repo / "fs/Makefile").write_text("a\n", encoding="utf-8")
        (repo / "fs/proc/task_mmu.c").write_text("VEUX divergent task_mmu\n", encoding="utf-8")
        (repo / "kernel/sys.c").write_text("VEUX divergent sys\n", encoding="utf-8")
        patch = repo / "filtered.patch"
        patch.write_text(stripped, encoding="utf-8")
        run(["git", "init", "-q"], cwd=repo)
        run(["git", "apply", "--check", str(patch)], cwd=repo)
        run(["git", "apply", str(patch)], cwd=repo)
        if (repo / "fs/Makefile").read_text(encoding="utf-8") != "b\n":
            die("selftest filtered donor git-apply result mismatch")
        if (repo / "fs/proc/task_mmu.c").read_text(encoding="utf-8") != "VEUX divergent task_mmu\n":
            die("selftest filtered donor unexpectedly changed task_mmu")
        if (repo / "kernel/sys.c").read_text(encoding="utf-8") != "VEUX divergent sys\n":
            die("selftest filtered donor unexpectedly changed kernel/sys")

    with tempfile.TemporaryDirectory(prefix="susfs-v2-divergent-port-selftest-") as td:
        root = Path(td)
        (root / "fs/proc").mkdir(parents=True)
        (root / "kernel").mkdir(parents=True)
        (root / "fs/proc/task_mmu.c").write_text(
            "#include <linux/pkeys.h>\n\n#include <asm/elf.h>\n"
            "\tm->count += len;\n\treturn 0;\n}\n\nstatic void\nshow_map_vma(struct seq_file *m, struct vm_area_struct *vma)\n"
        ...
        (root / "kernel/sys.c").write_text(
            "\treturn ret;\n}\n\nSYSCALL_DEFINE1(newuname, struct new_utsname __user *, name)\n"
            "{\n\tstruct new_utsname tmp;\n\tstruct task_struct *t;\n\tbool is_netmgrd = false;\n\n"
            "\tdown_read(&uts_sem);\n\tmemcpy(&tmp, utsname(), sizeof(tmp));\n\tup_read(&uts_sem);\n"
            "\tis_netmgrd = !strcmp(t->comm, \"netmgrd\");\n}\n"
            "static void unrelated_uname_snapshot(void)\n"
            "{\n\tstruct new_utsname tmp;\n"
            "\tdown_read(&uts_sem);\n\tmemcpy(&tmp, utsname(), sizeof(tmp));\n\tup_read(&uts_sem);\n}\n",
            encoding="utf-8",
        )
        port_task_mmu_veux(root)
        port_kernel_sys_veux(root)
        mmu = read(root / "fs/proc/task_mmu.c")
        sysc = read(root / "kernel/sys.c")
        if 'if (strstr(path, "lineage"))' not in mmu or "susfs_pagemap_unlock:" not in mmu:
            die("selftest VEUX task_mmu preservation/port failed")
        if 'is_netmgrd = !strcmp(t->comm, "netmgrd");' not in sysc or "susfs_spoof_uname(&tmp);" not in sysc:
            die("selftest VEUX kernel/sys preservation/port failed")
        original_uname_anchor = "\tdown_read(&uts_sem);\n\tmemcpy(&tmp, utsname(), sizeof(tmp));\n\tup_read(&uts_sem);\n"
        if sysc.count(original_uname_anchor) != 1 or sysc.count("susfs_spoof_uname(&tmp);") != 1:
            die("selftest scoped newuname anchor regression failed")

    with tempfile.TemporaryDirectory(prefix="susfs-v2-selftest-") as td:
        root = Path(td)
        vk = root / "kernel"
    ...
    with tempfile.TemporaryDirectory(prefix="susfs-v2-seven-hook-selftest-") as td:
        root = Path(td)
        k = root / "kernel"
        donor = root / "donor"
    ...
    # Pin the ABI/control-flow contracts that distinguish this V2 from the
    ...
    if len(VEUX_TASK_MMU_BLOB) != 40 or len(VEUX_KERNEL_SYS_BLOB) != 40:
        die("selftest VEUX divergent preimage contract failed")
    if VEUX_TASK_MMU_BLOB == VEUX_KERNEL_SYS_BLOB:
        die("selftest VEUX divergent preimage blobs unexpectedly identical")

    with tempfile.TemporaryDirectory(prefix="resukisu-r1-manager-rcu-selftest-") as td:
        k = Path(td)
        p = k / ".resukisu/kernel/manager/manager.c"
        ...
        verify_resukisu_manager_rcu_fix(k)

    if EXPECTED_KSU_VERSION != 35157 or EXPECTED_KSU_UAPI != 4:
        die("selftest ReSukiSU identity contract failed")
    if set(HOOK_SURFACE_BLOBS) != {
        "fs/exec.c", "fs/open.c", "fs/read_write.c", "fs/stat.c",
        "drivers/input/input.c", "kernel/reboot.c", "kernel/sys.c",
    }:
        die("selftest hook-surface preimage path contract failed")
    if any(len(v) != 40 for v in HOOK_SURFACE_BLOBS.values()):
        die("selftest hook-surface blob SHA contract failed")

    print("DONOR_STATFS_SEMANTIC_FIX=PASS")
    print("UPSTREAM_STATFS_COMPILE_FIX_BACKPORT=PASS")
    print("UPSTREAM_CLONE_MNT_RACE_FIX_BACKPORT=PASS")
    print("SUSFS_5_4_CORE_COHERENCE=PASS")
    print("VEUX_DIVERGENT_PREIMAGE_CONTRACT=PASS")
    print("HOOK_SURFACE_PREIMAGE_CONTRACT=PASS")
    print("RESUKISU_RCU_FIX_CONTRACT=PASS")
    print("RESUKISU_IDENTITY_CONTRACT=PASS")
    print("VEUX_DIVERGENT_OUTER_PORT=PASS")
    print("RUN35344193301_REGRESSION=PASS")
    print("RUN35346290137_REGRESSION=PASS")
    print("RUN35350262681_REGRESSION=PASS")
    ...
    print("NEWUNAME_SCOPED_ANCHOR=PASS")
    print("SELFTEST=PASS")
...
def main() -> None:
    ...
    if not all((args.kernel, args.resukisu, args.donor, args.susfs, args.baseline_commit)):
        ap.error("kernel, resukisu, donor, susfs roots and --baseline-commit are required unless --selftest is used")
    ...
    snapshot = verify_real(project_root, kernel, resukisu, donor, susfs, args.baseline_commit)
