...
ORIGIN_SOURCE_COMMIT = "fb3bbb10bc0480282c01b88ccaf39c0145cd9f51"
...
def patch_tree(kernel: Path, resukisu_root: Path) -> None:
    ...
    if vendor.exists() or vendor.is_symlink():
        die(f"vendored ReSukiSU target already exists: {vendor}")
    if not (resukisu_root / ".git").exists():
        die(f"ReSukiSU git metadata missing: {resukisu_root / '.git'}")
    if not (resukisu_root / "kernel" / "Kbuild").is_file():
        die(f"ReSukiSU kernel Kbuild missing: {resukisu_root / 'kernel' / 'Kbuild'}")

    # Preserve the complete pinned ReSukiSU git worktree. Current upstream
    # Kbuild requires KSU_SRC/../.git and intentionally rejects copied
    # kernel-only trees.
    ...
    if dst.resolve() != vendored_kernel.resolve():
        die("drivers/kernelsu symlink does not resolve to vendored ReSukiSU kernel")

    # Remove module_load_filter from the vendored ReSukiSU kernel tree.
    replace_once(
    ...
    ensure_absent(vendored_kernel, ["module_load_filter", "ksu_block_modules", "block_modules"],
...
def verify_real(project_root: Path, kernel: Path, resukisu: Path) -> Path:
    ...
    expected_baseline = os.environ.get("VEUX293_BASELINE_COMMIT", "").strip()
    if not expected_baseline:
        die("VEUX293_BASELINE_COMMIT is required")
    if actual_source != expected_baseline:
        die(f"5.4.293 baseline commit mismatch: expected {expected_baseline}, got {actual_source}")
    ...
    if run(["git", "-C", str(kernel), "status", "--porcelain"], capture=True):
        die("kernel baseline worktree is not clean")
    ...
    run([
        ...
        str(kernel), actual_source, KERNEL_VERSION, DEFCONFIG
    ])
    run([
...
def finalize_real(project_root: Path, kernel: Path, snapshot: Path) -> None:
    ...
    # Host tree delta is intentionally narrow and must never grow silently.
    expected_host_delta = {
        "drivers/Kconfig",
        "drivers/Makefile",
        "fs/exec.c",
    ...
    actual_host_delta = set(
        filter(None, run(["git", "-C", str(kernel), "diff", "--name-only"], capture=True).splitlines())
    )
    if actual_host_delta != expected_host_delta:
        die(
            "unexpected host delta: expected "
            + ",".join(sorted(expected_host_delta))
            + " actual "
            + ",".join(sorted(actual_host_delta))
        )

    vendor = kernel / ".resukisu"
    if not (vendor / ".git").exists():
        die("vendored ReSukiSU .git metadata missing after integration")
    vendor_head = run(["git", "-C", str(vendor), "rev-parse", "HEAD"], capture=True)
    if vendor_head != RESUKISU_COMMIT:
        die(f"vendored ReSukiSU HEAD mismatch: {vendor_head}")
    ...
    actual_resukisu_delta = set(
        filter(None, run(["git", "-C", str(vendor), "diff", "--name-only"], capture=True).splitlines())
    )
    if actual_resukisu_delta != expected_resukisu_delta:
        die(
            "unexpected ReSukiSU delta: expected "
            + ",".join(sorted(expected_resukisu_delta))
            + " actual "
            + ",".join(sorted(actual_resukisu_delta))
        )
    ...
    print(f"BASELINE_COMMIT={run(['git', '-C', str(kernel), 'rev-parse', 'HEAD'], capture=True)}")
    print(f"ORIGIN_SOURCE_COMMIT={ORIGIN_SOURCE_COMMIT}")
...
def selftest() -> None:
    with tempfile.TemporaryDirectory(prefix="veux-resukisu-selftest-") as td:
        ...
        # Use real local git repositories. This specifically reproduces the
        # upstream Kbuild requirement that KSU_SRC/../.git must exist and that
        # rev-list/describe/diff-index must work from the copied worktree.
        for repo in (k, r):
            run(["git", "init", "-q"], cwd=repo)
            run(["git", "config", "user.email", "selftest@example.invalid"], cwd=repo)
            run(["git", "config", "user.name", "VEUX Selftest"], cwd=repo)
            run(["git", "add", "-A"], cwd=repo)
            run(["git", "commit", "-q", "-m", "fixture"], cwd=repo)
        run(["git", "tag", "v4.2.0"], cwd=r)
        original_rs_head = run(["git", "-C", str(r), "rev-parse", "HEAD"], capture=True)
        ...
        assert (k/"drivers/kernelsu").is_symlink()
        assert (k/"drivers/kernelsu").resolve() == (k/".resukisu/kernel").resolve()
        assert (k/".resukisu/.git").exists()
        assert not (k/".resukisu/.git/shallow").exists()
        ensure_absent(k/".resukisu/kernel",
                      ["module_load_filter", "ksu_block_modules", "block_modules"],
                      "selftest module filter gate")

        # Reproduce the exact metadata operations performed by current ReSukiSU Kbuild.
        ksu_src = (k/"drivers/kernelsu").resolve()
        assert (ksu_src.parent/".git").exists()
        assert run(["git", "-C", str(ksu_src), "rev-parse", "HEAD"], capture=True) == original_rs_head
        assert int(run(["git", "-C", str(ksu_src), "rev-list", "--count", "HEAD"], capture=True)) >= 1
        assert run(["git", "-C", str(ksu_src), "describe", "--abbrev=0", "--tags"], capture=True) == "v4.2.0"
        dirty_rc = subprocess.run(["git", "-C", str(ksu_src), "diff-index", "--quiet", "HEAD"]).returncode
        assert dirty_rc == 1

        expected_host_delta = {
            "drivers/Kconfig", "drivers/Makefile", "fs/exec.c", "fs/open.c",
            "fs/stat.c", "kernel/reboot.c", "security/selinux/selinuxfs.c",
        }
        actual_host_delta = set(filter(None, run(["git", "-C", str(k), "diff", "--name-only"], capture=True).splitlines()))
        assert actual_host_delta == expected_host_delta

        expected_rs_delta = {
            "kernel/Kbuild", "kernel/core/init.c",
            "kernel/feature/module_load_filter.c",
            "kernel/feature/module_load_filter.h",
        }
        actual_rs_delta = set(filter(None, run(["git", "-C", str(k/".resukisu"), "diff", "--name-only"], capture=True).splitlines()))
        assert actual_rs_delta == expected_rs_delta
    ...
    print("RESUKISU_GIT_WORKTREE_LAYOUT=PASS")
    print("KBUILD_GIT_METADATA=PASS")
    print("HOST_DELTA_FIXTURE=PASS")
    print("RESUKISU_DELTA_FIXTURE=PASS")
