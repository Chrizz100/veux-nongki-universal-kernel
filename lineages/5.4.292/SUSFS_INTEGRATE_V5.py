...
# Exact audited Git blob identities for all seven ReSukiSU SUSFS inline-hook
# host surfaces.  The source commit is already pinned globally, but these
# per-file contracts make the actual hook inputs explicit and fail closed.
HOOK_SURFACE_BLOBS = {
...
def replace_once_in_region(
    ...
    """Replace one anchor only inside one explicitly delimited source region.

    This prevents a globally duplicated sequence elsewhere in the same C file
    from making an otherwise valid transform ambiguous.  Both region markers
    must be unique in the required ordering, and the old anchor must occur
    exactly once inside that region.
    """
    data = read(path)
    if data.count(region_start) != 1:
        die(f"{label}: region start count in {path}: {data.count(region_start)}")
    start = data.index(region_start)
    end = data.find(region_end, start + len(region_start))
    if end < 0:
        die(f"{label}: region end not found after start in {path}")
    region = data[start:end]
    count = region.count(old)
    if count != 1:
        die(f"{label}: expected exactly one anchor inside region in {path}, found {count}")
    region = region.replace(old, new, 1)
    write(path, data[:start] + region + data[end:])
...
def backport_clone_mnt_race_fix(kernel: Path) -> None:
    """Backport upstream SUSFS commit 04a9d713 clone_mnt race fix to 5.4.

    Match the semantic clone_mnt flag block, not donor indentation.  Run
    35352162500 proved the 5.4 donor uses a two-tab marker line while the
    previous fixture incorrectly used three tabs.
    """
    ...
    for marker in (
        ...
        if region.count(marker) != 1:
            die(f"clone_mnt race-fix precondition mismatch for {marker!r}: {region.count(marker)}")
    ...
    if "if (unlikely(is_mnt_ksu_unshared))" in middle:
        ...
        missing = [m for m in old_markers if m not in middle]
        if missing:
    ...
    final = read(path)
    final_region = final[fn_start:fn_start + len(region)]
    ...
    if final_region.count("static_branch_unlikely(&susfs_is_sdcard_android_data_not_decrypted)") != 1:
        # One static-key check must remain earlier in clone_mnt for allocation
        # routing.  Only the later mnt_flags write must stop depending on it.
...
def port_kernel_sys_veux(kernel: Path) -> None:
    ...
    Run 35346290137 proved that the utsname copy sequence is not globally
    unique in the real VEUX file.  Preserve the complete legacy-uname suffix
    byte-for-byte and scope the transformation only to newuname().
    """
    path = kernel / "kernel/sys.c"
    before = read(path)
    legacy_marker = "#ifdef __ARCH_WANT_SYS_OLD_UNAME\n"
    if before.count(legacy_marker) != 1:
        die(f"VEUX uname legacy marker count mismatch: {before.count(legacy_marker)}")
    legacy_suffix_before = before[before.index(legacy_marker):]
    ...
    legacy_suffix_after = data[data.index(legacy_marker):]
    if legacy_suffix_after != legacy_suffix_before:
        die("VEUX uname port modified legacy uname suffix")
    newuname_region = data[data.index("SYSCALL_DEFINE1(newuname,"):data.index(legacy_marker)]
    if newuname_region.count("susfs_spoof_uname(&tmp);") != 1:
        die("VEUX newuname scoped SUSFS call postcondition failed")
...
def semantic_gates(kernel: Path) -> None:
    ...
    # ReSukiSU 5.4 ABI contracts used by the host declarations.
    ...
    for marker in (
        ...
        if marker not in runtime_c:
            die(f"ReSukiSU SUSFS static-key definition missing: {marker}")
    ...
    selinuxfs = read(kernel / "security/selinux/selinuxfs.c")
    if "static ssize_t (*const write_op[])(struct file *, char *, size_t)" in selinuxfs:
        die("SELinux write_op remained static")
    if "ssize_t (*const write_op[])(struct file *, char *, size_t)" not in selinuxfs:
        die("SELinux write_op exported form missing")
    if "static const struct file_operations sel_handle_status_ops" in selinuxfs:
        die("SELinux sel_handle_status_ops remained static")
    if "const struct file_operations sel_handle_status_ops" not in selinuxfs:
...
def selftest() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="susfs-v3-statfs-compile-selftest-") as td:
        root = Path(td)
    ...
    with tempfile.TemporaryDirectory(prefix="susfs-v5-clone-race-selftest-") as td:
        root = Path(td)
        (root / "fs").mkdir(parents=True)
    ...
    with tempfile.TemporaryDirectory(prefix="susfs-v3-core-coherence-selftest-") as td:
        root = Path(td)
        k = root / "kernel"
        u = root / "upstream"
    ...
    with tempfile.TemporaryDirectory(prefix="susfs-v3-filtered-git-apply-selftest-") as td:
        repo = Path(td)
    ...
    with tempfile.TemporaryDirectory(prefix="susfs-v3-divergent-port-selftest-") as td:
        root = Path(td)
        ...
        if 'is_netmgrd = !strcmp(t->comm, "netmgrd");' not in sysc:
            die("selftest VEUX kernel/sys lost netmgrd qgki rule")
        if sysc.count("susfs_spoof_uname(&tmp);") != 1:
            die("selftest VEUX newuname scope regression failed")
        old_uname_region = sysc.split("#ifdef __ARCH_WANT_SYS_OLD_UNAME\n", 1)[1]
        if "susfs_spoof_uname(&tmp);" in old_uname_region:
            die("selftest VEUX old uname was incorrectly modified")

    with tempfile.TemporaryDirectory(prefix="susfs-v3-selftest-") as td:
        root = Path(td)
        vk = root / "kernel"
    ...
    with tempfile.TemporaryDirectory(prefix="susfs-v3-seven-hook-selftest-") as td:
        root = Path(td)
        k = root / "kernel"
        donor = root / "donor"
