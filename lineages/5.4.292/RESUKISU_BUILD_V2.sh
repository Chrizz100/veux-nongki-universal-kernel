...
# VEUX 5.4.292 ReSukiSU compile V2
...
INTEGRATOR="$PROJECT_ROOT/lineages/5.4.292/RESUKISU_INTEGRATE_V2.py"
INTEGRATOR_EXPECTED_SHA256="6f75a36d540fcef818d5c49ee0ed1b8fc322431d702fab08cd0a07804cfb3780"

WORK_ROOT="${1:-$PROJECT_ROOT/.work/5.4.292-resukisu-v2}"
...
[[ ! -f "$RS/.git/shallow" ]] || {
  echo "ERROR: ReSukiSU checkout is unexpectedly shallow" >&2
  exit 24
}
...
[[ -L "$SRC/drivers/kernelsu" ]] || {
  echo "ERROR: drivers/kernelsu is not a symlink" >&2
  exit 25
}
[[ -d "$SRC/.resukisu/.git" ]] || {
  echo "ERROR: vendored ReSukiSU git metadata missing" >&2
  exit 26
}
[[ "$(git -C "$SRC/.resukisu" rev-parse HEAD)" == "$RESUKISU_COMMIT" ]] || {
  echo "ERROR: vendored ReSukiSU commit mismatch" >&2
  exit 27
}
[[ "$(readlink -f "$SRC/drivers/kernelsu")" == "$(readlink -f "$SRC/.resukisu/kernel")" ]] || {
  echo "ERROR: drivers/kernelsu symlink target mismatch" >&2
  exit 28
}
...
echo "ReSukiSU V2 compile finished successfully."
