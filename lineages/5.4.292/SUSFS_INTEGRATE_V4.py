...
def backport_clone_mnt_race_fix(kernel: Path) -> None:
    ...
    for marker in (
        ...
        if data.count(marker) != 1:
            die(f"clone_mnt race-fix precondition mismatch for {marker!r}: {data.count(marker)}")
    ...
    if old_count == 1 and new_count == 0:
        ...
        die(f"clone_mnt race-fix block state mismatch: old={old_count}, new={new_count}")

    final = read(path)
    if final.count("if (unlikely(is_mnt_ksu_unshared))") != 1:
        die("clone_mnt race-fix postcondition missing captured boolean check")
    if final.count("mnt->mnt.mnt_flags |= VFSMOUNT_MNT_FLAGS_KSU_UNSHARED_MNT;") < 1:
        die("clone_mnt race-fix postcondition missing unshared marker")
...
def selftest() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="susfs-v4-clone-race-selftest-") as td:
        root = Path(td)
        (root / "fs").mkdir(parents=True)
